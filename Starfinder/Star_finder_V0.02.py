...
class Star_finder(tk.Tk):
    ...
    def set_var(self):
        ...
        self.NGC = pd.read_csv('catalogs/ngc-ic2000new.csv') #NGC
        
        #Clean datasets
        #self.messier.drop(['Picture', 'Distance(kly)','Apparent magnitude'], axis = 1, inplace = True) #Drop all useless columns
        ...
        self.NGC.drop(['Const.', 'Unnamed: 7', 'Unnamed: 9', 'Unnamed: 13']
                       , axis = 1, inplace = True) #Drop all useless columns
...
# ---------------------------------- BackEnd --------------------------------- #
    ...
    def update_catalog(self,x) :
        if self.slct_catalog.get() == '--Select a Catalog--':
            ...
            print(f'No Catalog : {self.current_catalog}') #Set current catalog to 'No catalog'
            self.current_catalog = 'None'
    ...
    def update_target(self,x):
        ...
        if self.current_catalog == 'Messier':
            coordinate = (['RA : ',self.messier['Right ascension'][self.current_target]],['Decl ',self.messier['Declination'][self.current_target]]) #Get the coordinates tuple
        ...
        if self.current_catalog == 'NGC':
            RA = ['RA : ',self.NGC['Rah'][self.current_target],'h',  self.NGC['Ram'][self.current_target],'m', self.NGC['Ras'][self.current_target],'s']
            Decl =[self.NGC['Decldeg'][self.current_target], '°', self.NGC['Declm'][self.current_target], '"', self.NGC['Decls'][self.current_target],"'"]
            coordinate = (RA,Decl)
            print(self.current_target)
            print(type(self.current_target))
            self.target_coords.set(coordinate)
